...
namespace Aop.Api.Domain
{
    ...
    {
        /// <summary>
        /// 条件类型 QUOTA_TOTAL(有效期总额度), QUOTA_DAY(日额度), QUOTA_MONTH(月额度), QUOTA_ONCE(单笔额度)， ALARM_CLOCK_TIME(闹钟时间), CARD_TYPE(卡类型), COMBINED_PERIOD_QUOTA(多时间段额度), PERIOD_TIME(多时间段), PERIOD_QUOTA(时间段额度), MCC(商户类型), DISTRICT(消费区域限制), MERCHANT(商户), ISSUE_CITY(发放城市), AREA(坐标围栏), TAKE_AWAY_CATEGORY(外卖类目), MEAL_MERCHANT(可选商户限制)， SHIPPING_ADDRESS(收餐地址)， BOOK_SWITCH(是否开启预订)， MARKED_WORDS(提示单文案)， SUPPLIER_CHANNEL(供应商渠道)， APPROACH(使用途径)， ELM_MERCHANT(饿了么商户), MULT_ISSUE_CITY(多发放城市)
        /// </summary>
