...
namespace Aop.Api.Domain
{
    ...
    public class AlipayCommerceRentOrderAftersaleCreateModel : AopObject
    {
        ...
        /// <summary>
        /// 发起售后原因，具体传入规则如下： 1. 当aftersale_type=ORDER_CANCEL时，根据当前订单状态是「待发货」和「已发货」，可传入的原因类型不同： 1.1. 已发货：可传入TIMEOUT_FOR_CONFIRM、LOGISTICS_ILLEGAL、REJECT_TO_CONFIRM、NO_NEED、BUYER_AGREED、OTHER； 1.2. 待发货：可传入OTHER_CHANNEL_CHEAP、CHEAPER_ITEM、RENT_AMOUNT_OVER_PLAN、NO_NEED、NO_GOODS_OR_REJECT、NEED_TO_CHANGE_CHANNEL、NEED_TO_PAY_MANY_PERIOD_AMOUNT、CANNOT_CAONTACT_WITH_MERCHANT、OTHER。 2. 当aftersale_type= COMPENSATION时，如果pay_items中金额类型为INDEMNITY，则可以传入ITEM_DAMAGED、ITEM_REPAIR、ITEM_LOST和ITEM_DEPRECIATION；如果金额类型为LATE_FEE，则可以传入RETURN_EARLY和RETURN_OVERDUE。
        /// </summary>
