...
namespace Aop.Api.Domain
{
    ...
    public class AlipayUserDigitalidentityCertificateSyncModel : AopObject
    {
        ...
        /// <summary>
        /// 含义：凭证ID，同一自然人同一凭证多次申领时要求该ID唯一且幂等相同。  如何设置：（区分是否为【身份码】和【非身份码】主要看凭证实例标识是否以IDCODE结尾，可见文档开头「注意」说明） 【身份码】：身份证号码，用于系统内唯一性标示还被用于页面展示，由机构侧传入；  【非身份码】：证件号，发证机构生成的证件号，要求对C端用户可理解，如：行驶证传车牌号、驾驶证传身份证号码、结婚证传结婚证号等，除用于唯一性标示外还被用于页面展示，由机构侧传入。
        /// </summary>
